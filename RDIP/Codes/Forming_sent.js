...
"है वहाँ बड़ी सी एक किताब "]];
    ...
    if(x == "english")
    {
      var grpsent = English_Sentence[Math.floor(Math.random()*English_Sentence.length)] ;
      var sent = grpsent[Math.floor(Math.random()*grpsent.length)];
      var word = sent.split(" ");
        var len = word.length;
          word.pop();
      for(initial=0;initial<len;initial++)
      {
        ...
        word[position] = word[word.length-1];
        word[word.length-1] = temp;
         var words = document.createElement("BUTTON"); 
         words.style.marginLeft = "30px"
         words.innerHTML = word[word.length-1];
      document.getElementById("form_3").appendChild(words);
}
     
    }
    else if (x == "hindi")
    {
      var grpsenth = Hindi_Sentence[Math.floor(Math.random()*Hindi_Sentence.length)] ;
      var senth = grpsenth[Math.floor(Math.random()*grpsenth.length)];
      var wordh = senth.split(" ");
     var len = wordh.length;
      wordh.pop();
      for(initial=0;initial<len;initial++)
      {

        var position = Math.floor(Math.random()*wordh.length);
        var temp = wordh[position];
        wordh[position] = wordh[wordh.length-1];
        wordh[wordh.length-1] = temp;
         var words = document.createElement("BUTTON"); 
         words.style.marginLeft = "30px"
         words.style.margin = "center"
         words.innerHTML = wordh[wordh.length-1];
      document.getElementById("form_3").appendChild(words);
}
